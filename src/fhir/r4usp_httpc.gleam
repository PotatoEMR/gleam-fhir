    ...
    Ok(resp) ->
      case r4usp_sansio.http_or_operationoutcome_resp(resp) {
        ...
        Error(err) ->
          Error(
            ErrSansio(case err {
              r4usp_sansio.ErrParseJson(e) -> ErrParseJson(e)
              r4usp_sansio.ErrNotJson(e) -> ErrNotJson(e)
              r4usp_sansio.ErrOperationoutcome(e) -> ErrOperationoutcome(e)
            }),
          )
      }
  }
...
///
/// r4usp_httpc.search_any("name=e&_count=25", "Patient", client) |> r4usp_httpc.all_pages(client)
pub fn all_pages(
  first_bundle: Result(r4usp.Bundle, Err),
  client: FhirClient,
) -> Result(r4usp.Bundle, Err) {
  case all_pages_loop(first_bundle, [], client) {
    ...
    Ok(#(last_bundle, bundles)) -> {
      ...
      Ok(r4usp.Bundle(..last_bundle, entry: entries, link: []))
    }
  }
}
...
pub fn all_pages_loop(
  curr_bundle: Result(r4usp.Bundle, Err),
  acc_bundles: List(r4usp.Bundle),
  client: FhirClient,
) -> Result(#(r4usp.Bundle, List(r4usp.Bundle)), Err) {
  case curr_bundle {
    Error(err) -> Error(err)
    Ok(curr_bundle) -> {
      let acc_bundles = [curr_bundle, ..acc_bundles]
      case r4usp_sansio.bundle_next_page_req(curr_bundle, client) {
