...
A choice element can be one of multiple types, so in Gleam they are custom types with multiple variants. For example, Patient.multipleBirth[x] can be either multipleBirthBoolean or multipleBirthInteger, so in Gleam `resources.PatientMultiplebirth` can be `PatientMultiplebirthBoolean(multiple_birth: Bool)` or `PatientMultiplebirthInteger(multiple_birth: Int)`.
