...
Without knowing an extension's urls or StructureDefinition ahead of time, the normal [Extension](https://hexdocs.pm/fhir/resources/extension.html) still works, but is somewhat verbose and requires string url keys in order to support extensions of any type. If extensions are known when the the library code is generated, decoders will instead parse them into custom types specfic to that extension. For example as r4us is generated with http://hl7.org/fhir/StructureDefinition/individual-recordedSexOrGender `r4us.patient_decoder()` parses that extension into its specific type that allows `rsg.source_document`, `rsg.value`, etc. These custom types for extensions allow shorter code without string url keys.
