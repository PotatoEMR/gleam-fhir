...
import fhir/r5/primitive_types as pt
...
pub fn extension_test() {
  ...
  let pat_exts: ct.ExtDict = ct.exts_to_extdict(pat.extension)
  let assert Ok([ident]) =
    dict.get(
      pat_exts.exts_by_url,
      "http://hl7.org/fhir/StructureDefinition/individual-genderIdentity",
    )
  let assert ct.ExtDictComplex(ident_exts) = ident.content

  let assert Ok([val]) = dict.get(ident_exts.exts_by_url, "value")
  let assert ct.ExtDictSimple(val_cc) = val.content
  let assert ct.ExtensionValueCodeableconcept(ident_cc) = val_cc
  ...
  let assert ct.ExtDictSimple(val_period) = when.content
  let assert ct.ExtensionValuePeriod(period) = val_period
  ...
  let assert Ok(ct.Extension(ext: ct.ExtComplex(ident_children), ..)) =
    list.find(pat.extension, fn(e) {
      e.url
      == "http://hl7.org/fhir/StructureDefinition/individual-genderIdentity"
    })
  let assert Ok(ct.Extension(
    ext: ct.ExtSimple(ct.ExtensionValueCodeableconcept(ident_cc)),
    ..,
  )) = list.find(ident_children, fn(e) { e.url == "value" })
  let assert [ident_coding] = ident_cc.coding
  assert ident_coding.code == Some("446141000124107")
  let assert Ok(ct.Extension(
    ext: ct.ExtSimple(ct.ExtensionValuePeriod(period)),
    ..,
